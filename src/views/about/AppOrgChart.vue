<template>
  <div class="min-h-screen bg-slate-50 p-4 font-sans relative overflow-hidden">
    ...
    <div class="relative z-10 max-w-7xl mx-auto">
      <header
        class="flex items-center justify-between pb-8 border-b border-slate-200 mb-10 relative z-30"
      >
        ...
        <div
          class="flex items-center gap-2 bg-white/50 p-2 rounded-full shadow-inner border border-slate-100"
        >
          <div
            class="w-10 h-10 rounded-full bg-purple-600 flex items-center justify-center text-white font-bold shadow"
          >
            D
          </div>
          <div
            class="w-10 h-10 rounded-full bg-pink-600 flex items-center justify-center text-white font-bold shadow"
          >
            M
          </div>
          <div
            class="w-10 h-10 rounded-full bg-orange-500 flex items-center justify-center text-white font-bold shadow"
          >
            S
          </div>
        </div>
      ...
      <div class="flex flex-col items-center">
        <div class="relative flex flex-col items-center mb-16 relative z-20">
          <div class="relative w-40 h-52 mb-5 group">
            <div
              class="absolute inset-0 rounded-2xl bg-gradient-to-b from-teal-400 to-teal-600 opacity-20 blur-xl group-hover:opacity-40 transition-opacity"
            ></div>
            <img
              :src="`/images/portraits/${directorData.imageUrl}`"
              :alt="directorData.name"
              class="relative z-10 w-full h-full object-cover rounded-2xl border-4 border-white shadow-2xl"
              @error="handlePortraitError"
            />
          </div>

          <div
            class="bg-gradient-to-r from-teal-600 to-teal-500 rounded-xl px-10 py-4 text-center shadow-lg border border-teal-400 relative z-20"
          >
            <h3 class="text-sm font-medium text-teal-100 uppercase tracking-wide">
          ...
          <div
            class="absolute top-full left-1/2 -translate-x-1/2 h-16 w-1 bg-teal-400/40 z-0"
          ></div>
        </div>

        <div
          class="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-6 relative z-10"
        >
          <div
            class="hidden lg:block absolute -top-16 left-1/2 -translate-x-1/2 w-[calc(100%-1.5rem)] h-1 bg-gradient-to-r from-purple-300 via-teal-300 to-orange-300 rounded-full opacity-50 z-0"
          ></div>
          ...
          >
            <div class="relative flex flex-col items-center mb-10 w-full relative z-10">
              <div class="relative w-32 h-44 mb-4 group">
                <div
                  class="absolute inset-0 rounded-2xl opacity-15 blur-lg transition-opacity group-hover:opacity-30"
                  :class="`bg-${mission.color}-500`"
                ></div>
                <img
                  :src="`/images/portraits/${mission.deputy.imageUrl}`"
                  :alt="mission.deputy.name"
                  class="relative z-10 w-full h-full object-cover rounded-2xl border-4 border-white shadow-xl"
                  @error="handlePortraitError"
                />
              </div>

              <div
                class="w-full rounded-2xl shadow-lg border p-5 text-center transition-transform hover:scale-[1.02] relative z-10"
                :class="`bg-${mission.color}-100/30 border-${mission.color}-200`"
              >
                <p class="text-base font-semibold" :class="`text-${mission.color}-900`">
                  {{ mission.deputy.name }}
                </p>
                <p class="text-sm mt-1" :class="`text-${mission.color}-800`">
                  {{ mission.deputy.title }}
                </p>
              </div>

              <div
                class="absolute top-full left-1/2 -translate-x-1/2 h-10 w-1 opacity-50 z-0"
                :class="`bg-${mission.color}-400`"
              ></div>
            </div>

            <div
              class="w-full rounded-2xl shadow-inner p-6 flex-grow border relative z-10"
              :class="`bg-${mission.color}-50 border-${mission.color}-100`"
            >
              <div
                v-if="mission.subHeader"
                class="text-center mb-6 py-2 px-4 rounded-lg bg-white/70 border"
                :class="`border-${mission.color}-200`"
              >
                <p class="text-sm font-medium text-slate-700">{{ mission.subHeader.line1 }}</p>
                <p v-if="mission.subHeader.line2" class="text-sm text-slate-600">
                  {{ mission.subHeader.line2 }}
                </p>
              </div>

              <div v-for="(group, gIndex) in mission.groups" :key="gIndex" class="mb-5 last:mb-0">
                <h4
                  class="text-sm font-semibold text-slate-900 border-b pb-2 mb-3"
                  :class="`border-${mission.color}-200`"
                >
                  {{ group.header }}
                </h4>
                ...
                <ul class="space-y-2.5 custom-scrollbar h-auto max-h-[350px] overflow-y-auto pr-1">
                  ...
                  >
                    <div
                      class="flex items-start gap-1.5 leading-relaxed hover:bg-white/50 p-1 rounded transition-colors"
                    >
                    ...
                    <ul
                      ...
                      class="pl-7 mt-1.5 space-y-1 relative"
                    >
                      <div
                        class="absolute left-[7px] top-1 bottom-1 w-px border-l border-dotted"
                        :class="`border-${mission.color}-200`"
                      ></div>
            ...
            <div
              class="hidden lg:block absolute -top-16 left-1/2 -translate-x-1/2 w-1 h-16 opacity-30 z-0"
              :class="`bg-${mission.color}-400`"
            ></div>
          </div>
        </div>
...
const breadcrumbList = ref([
  ...
  { name: "โครงสร้างหน่วยงานโรงพยาบาลมะเร็งอุบลราชธานี" }, // ตัวสุดท้ายจะเป็นสีเข้มอัตโนมัติ
]);
...
const missions = ref([
  {
    ...
    color: "purple",
    deputy: {
      name: "พญ.ชุติวรรณ วิวัฒนาสิทธิพงศ์",
      title: "รองผู้อำนวยการภารกิจด้านวิชาการและการแพทย์",
      imageUrl: "deputy-1.png",
    },
    groups: [
      {
      ...
      },
      {
        ...
        items: [{ title: "8.1 งานด้านรังสีร่วมรักษา", subs: [] }],
      },
      {
  ...
  },
  {
    ...
    color: "green",
    deputy: {
      name: "ดร.ชลิยา วามะลุน",
      title: "รองผู้อำนวยการภารกิจด้านการพยาบาล",
      imageUrl: "deputy-2.png",
    },
  ...
  },
  {
    ...
    color: "blue",
    deputy: {
      name: "นายอรรถวิทย์ ภักดี",
      title: "รองผู้อำนวยการภารกิจด้านอำนวยการ",
      imageUrl: "deputy-3.png",
    },
    ...
    groups: [
      ...
      },
      {
        ...
        items: [
          ...
          {
            title: "2.3 งานพัสดุและงานบำรุงรักษา",
            subs: [
              "หมวดพัสดุ",
              "หมวดคลัง",
              "หมวดซ่อมบำรุง",
  ...
  },
  {
    ...
    color: "orange",
    deputy: {
      name: "พญ.ดาริกา ดาโรจน์",
      title: "รองผู้อำนวยการภารกิจด้านพัฒนาระบบสุขภาพ",
      imageUrl: "darika.png",
    },
    groups: [
      {
      ...
      },
      {
        ...
        items: [{ title: "3.1 งานสำนักงานการแพทย์เขตสุขภาพ", subs: [] }],
      },
      {
        header: "4. กลุ่มงานพพัฒนาคุณภาพ",
        items: [{ title: "4.1 งานพัฒนาคุณภาพ", subs: [] }],
      },
    ],
  },
]);
