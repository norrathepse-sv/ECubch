...
const goToNewsDetail = (id: number | string) => {
  router.push(`/news/${id}`);
};
...
<template>
  <div
    class="relative w-full h-[300px] md:h-[400px] lg:h-[500px] overflow-hidden bg-transparent space-y-0 mb-16 group flex items-center"
    ...
    >
      ...
      >
        ...
        <div class="relative w-full h-full overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.2)]">
          ...
          <div
            class="absolute inset-0 flex flex-col justify-end text-left p-8 md:p-12 z-20 overflow-hidden"
          >
            <h2
              v-if="slide.title"
            ...
            >
              <p
                class="text-base md:text-lg font-light text-gray-200 leading-relaxed line-clamp-2 max-w-xl opacity-90"
              >
                {{ slide.subtitle }}
              </p>

              <button
                @click="goToNewsDetail(slide.id)"
                class="mt-4 text-sm font-bold text-navy bg-gold hover:bg-white hover:text-navy px-6 py-2 rounded-full transition-colors drop-shadow-md w-fit flex items-center gap-2"
              >
